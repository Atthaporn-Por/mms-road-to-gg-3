...
class Caller(User):
    ...
    def searchTaxi(self):
        mysql = MySQL()
        min_latitude = self.user_position.latitude - 1.0
        max_latitude = self.user_position.latitude + 1.0
        min_longitude = self.user_position.longitude - 1.0
        max_longitude = self.user_position.longitude + 1.0
    ...
    def searchTaxi(self, latitude, longitude):
        mysql = MySQL()
        min_latitude = latitude - 1.0
        max_latitude = latitude + 1.0
        min_longitude = longitude - 1.0
        max_longitude = longitude + 1.0
        list_of_results = mysql.query('SELECT * FROM gettaxi.driver WHERE ((real_time_lat_location BETWEEN %s AND %s) AND (real_time_long_location BETWEEN %s AND %s))', (min_latitude,max_latitude,min_longitude,max_longitude))
        list_of_taxi = []
        for result in list_of_results:
            (phone, isActive, status, real_time_lat, real_time_long, license_no, ssn, driver_premission_id) = result
